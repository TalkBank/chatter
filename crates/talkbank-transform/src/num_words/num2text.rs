//! Number-to-text expansion for CHAT content.
//!
...
/// Em-dash / en-dash characters that typography (or an ASR provider) may
/// substitute for a plain hyphen inside a number token; both are normalized to
/// ASCII `-` before the dash-split. Named `\u{...}` constants per the house
/// rule that raw dash characters never appear in source.
...
pub fn expand_number(word: &str, lang: &str) -> String {
    ...
    // Handle dash-separated digit groups (e.g. "21-22", or em/en-dash-joined).
...
// ---------------------------------------------------------------------------
// English ordinal / decade helpers
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn test_em_dash_normalized() {
        // "5<em-dash>6"; the em-dash is functional test input, written as a
        // \u escape per the house no-raw-dash rule.
        let result = expand_number("5\u{2014}6", "eng");
