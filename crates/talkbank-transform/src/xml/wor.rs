...
//! float XML shape TalkBank XML emits. Keeping them together avoids
...
//! with `start_ms` / `end_ms` as `u64`). TalkBank XML emits
...
impl XmlEmitter {
    ...
    /// TalkBank XML emits this between the terminator `<t>` and any
    /// following `<wor>` tier.
    ...
    /// an untimed `<w>`?) needs to be pinned to the TalkBank XML output on a
    ...
    pub(super) fn emit_wor(&mut self, wor: &WorTier) -> Result<(), XmlWriteError> {
        ...
        for item in wor.items.iter() {
            match item {
                WorItem::Word(word) => {
                    ...
                    // but the TalkBank XML goldens always carry timing on
        ...
        // TalkBank XML closes every `<wor>` block with a terminator
...
/// decimal places, matching the TalkBank XML format.
