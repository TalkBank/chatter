...
//! and what must be refused, using the health of the utterance each edit
//! falls inside as the sole gate. An edit whose enclosing utterance parsed
//! clean is admitted; an edit whose enclosing utterance needed parser
//! recovery, carries no parse provenance at all, or lands outside every
//! utterance, is refused and reported with a reason instead of silently
//! applied.
//!
...
use super::engine::{EditProvenance, SpliceEdit};
...
/// Admit only those edits whose enclosing utterance parsed clean.
///
...
pub fn admit_edits(file: &ChatFile, edits: Vec<SpliceEdit>) -> Admission {
    ...
    for edit in edits {
        ...
        match utterance.parse_health {
            ...
            ParseHealthState::Tainted(_) => admission.skipped.push(Skipped {
                provenance: edit.provenance().clone(),
