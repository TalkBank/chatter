...
fn lint_directory(
    ...
    for entry in walkdir::WalkDir::new(dir) {
        ...
        // Skip non-transcripts and macOS AppleDouble `._*.cha` sidecars.
        if !is_chat_transcript_path(entry.path()) {
            continue;
        }
...
fn apply_fixes_to_file(source: &str, fixes: &[LintFix]) -> Result<String, String> {
    ...
    sorted_fixes.sort_by_key(|fix| std::cmp::Reverse(fix.byte_start));
