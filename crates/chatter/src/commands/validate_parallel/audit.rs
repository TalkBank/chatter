...
use talkbank_transform::validation_runner::{ValidationStatsSnapshot, is_chat_transcript_path};
...
fn discover_chat_files(path: &Path) -> Vec<std::path::PathBuf> {
    ...
    walkdir::WalkDir::new(path)
        ...
        // Skip macOS AppleDouble `._*.cha` sidecars (and non-transcripts).
        .filter(|entry| is_chat_transcript_path(entry.path()))
