...
///
/// The `chatter validate` CLI itself always constructs `Recursive` (a
/// single non-recursive file is handled by the file-mode dispatch above
/// this layer). `SingleFile` is part of the typed surface and is matched
/// by `runtime::run_validation_runtime`, but no current caller constructs
/// it; the `#[allow(dead_code)]` keeps the variant available for callers
/// that need the file-target mode without tripping the never-constructed
/// lint.
#[allow(dead_code)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
