...
pub fn chat_to_json_directory(
    ...
    // Collect all .cha files (skipping macOS AppleDouble `._*.cha` sidecars).
    let cha_files: Vec<PathBuf> = WalkDir::new(input_dir)
        .into_iter()
        .filter_map(|e| e.ok())
        .filter(|e| e.file_type().is_file() && is_chat_transcript_path(e.path()))
        .map(|e| e.into_path())
        .collect();
...
//
// TODO: bundle args into a ConvertConfig struct.
#[allow(clippy::too_many_arguments)]
fn convert_one_file(
    cha_path: &Path,
    input_dir: &Path,
    ...
    if !force
        && let (Ok(cha_meta), Ok(json_meta)) = (fs::metadata(cha_path), fs::metadata(&json_path))
        && let (Ok(cha_mtime), Ok(json_mtime)) = (cha_meta.modified(), json_meta.modified())
        && json_mtime >= cha_mtime
    {
