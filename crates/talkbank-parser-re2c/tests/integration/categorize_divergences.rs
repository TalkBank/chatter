...
///
/// Some variants (e.g. `WordDiff`) are not currently constructed by the
/// categorizer but are kept so the categorization taxonomy is complete and
/// future categorizer rules don't have to revisit the enum shape.
#[allow(dead_code)]
