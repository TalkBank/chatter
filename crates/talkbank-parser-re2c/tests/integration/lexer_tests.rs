...
use talkbank_parser_re2c::lexer::{
    ...
    COND_MAIN_CONTENT, COND_MOR_CONTENT, COND_PARTICIPANTS_CONTENT, COND_PHO_CONTENT,
...
#[test]
// Test-side: the (input, predicate) shape is more readable inline than
...
fn lex_corpus_main_tiers_no_panic() {
    // Resolve at runtime to whatever the developer's chatter checkout
    // happens to be. CARGO_MANIFEST_DIR is the talkbank-parser-re2c
    // crate dir; the reference corpus lives two levels up at the repo
    // root. On CI and in fresh clones the path always resolves; if it
    // doesn't, skip (covers exotic setups that build the crate
    // out-of-tree).
    let base = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../corpus/reference");
...
/// Regression: MSU03b.cha line counting must stay aligned with Tree-sitter.
#[test]
fn parse_file_msu03b_line_count() {
    let path = format!(
        "{}/data/aphasia-data/English/Protocol/MSU/PWA/MSU03b.cha",
        crate::fixture_utils::meta_repo_root().display()
    );
    ...
    assert_eq!(
        file.lines.len(),
        ts_file.lines.len(),
        "re2c line count should match Tree-sitter"
    );
}
