//! Unit tests: lex real lines from the wild-corpus tree at
//! `$TALKBANK_DATA` (see extract_fixtures.rs for the env var contract).
//!
...
///
/// Kept as an alternative collector parallel to `collect_lines` for tests that
/// classify by `ChatLineKind` rather than text prefix; not all kind-based tests
/// are written yet.
#[allow(dead_code)]
...
///
/// Kept as a reusable batch wrapper around `assert_clean_lex`; the current
/// corpus tests inline the loop instead, but new corpus tests should prefer
/// this helper.
#[allow(dead_code)]
