//! Producer-owned incremental source revisions.
use crate::TreeSitterParser;
use std::sync::Arc;
use talkbank_model::{ErrorSink, model::ChatFile};
use tree_sitter::Tree;

/// An exact source revision and its parser-produced reusable CST.
///
/// Fields are private: callers cannot install a tree from another source or
/// skip the edit required when moving to a new revision. This is source/cache
/// identity, not syntax validity; recovered input can also have a reusable CST.
...
impl ParsedRevision {
    /// The immutable source that produced this revision.
    pub fn source(&self) -> &str {
        &self.source
    }

    /// A detached tree for structural queries. Mutating it cannot change this
    /// revision, and no API can install it back into the revision.
...
impl TreeSitterParser {
    /// Parse a complete revision, deriving any incremental edit from its owner.
    ///
    /// The previous revision supplies both its tree and the source it parsed.
    /// The caller supplies only the new source, never an independent edit or
    /// tree. Skipped intermediate editor revisions therefore use the correct
    /// baseline. Diagnostics and recovery match streaming cold parsing.
    pub fn parse_chat_file_revision(
        &self,
        source: Arc<str>,
        previous: Option<&ParsedRevision>,
        errors: &impl ErrorSink,
    ) -> (ChatFile, ParsedRevision) {
        let old_tree = previous.and_then(|old| {
            old.tree.clone().map(|mut tree| {
                if let Some(edit) = compute_input_edit(&old.source, &source) {
                    tree.edit(&edit);
                }
                tree
            })
        ...
        let (file, tree) =
            self.parse_chat_file_streaming_incremental(&source, old_tree.as_ref(), errors);
        (file, ParsedRevision { source, tree })
    }
}
...
fn compute_input_edit(old_text: &str, new_text: &str) -> Option<tree_sitter::InputEdit> {
    if old_text == new_text {
        return None;
    }
    // Equal `char` values have equal UTF-8 widths. Summing complete producer
    // values yields a boundary in both inputs; no partial-code-point repair
    // state can arise. Restrict the suffix to the remaining source so it
    // cannot overlap the shared prefix.
    let start = common_character_bytes(old_text.chars(), new_text.chars());
    let suffix = common_character_bytes(
        old_text[start..].chars().rev(),
        new_text[start..].chars().rev(),
    );
...
/// Count bytes only from equal, complete Unicode scalar values.
fn common_character_bytes(
    old: impl Iterator<Item = char>,
    new: impl Iterator<Item = char>,
) -> usize {
    old.zip(new)
