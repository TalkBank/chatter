...
impl TuiState {
    ...
    /// Auto-scroll so the selected error is visible. When the error
    /// is outside the viewport in either direction, scroll so the
    /// error's start_line sits at the top.
    fn scroll_to_selected_error(&mut self) {
        if let Some(&start_line) = self.scroll.error_line_starts.get(self.selected_error_idx) {
            ...
            let off_top = start_line < self.scroll.offset;
            let off_bottom = start_line >= viewport_end;
    ...
    pub fn open_in_clan(&self) -> Result<(), OpenInClanError> {
        ...
        // Shared with the desktop GUI: same resolved coordinates, same canonical
        // timeout, same bare `error.message` highlight, so the two surfaces issue
        // the identical CLAN request.
        send2clan::open_location_in_clan(
