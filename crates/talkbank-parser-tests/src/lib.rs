...
/// From a checkout of the `tree-sitter-grammar-utils` repository (cwd
/// = that repo's root), run the `generate_traversal` example pointed
/// at this repo's `grammar/src/grammar.json` and
/// `grammar/src/node-types.json`, with `--skip whitespaces`, and
/// redirect stdout to this file's path:
///
/// ```sh
/// cargo run --example generate_traversal \
///   -p tree-sitter-node-types -- \
///   <CHATTER>/grammar/src/grammar.json \
///   <CHATTER>/grammar/src/node-types.json \
///   --skip whitespaces \
///   > <CHATTER>/crates/talkbank-parser-tests/src/generated_traversal.rs
/// ```
///
/// where `<CHATTER>` is the path to your chatter checkout.
///
/// The generated file is standalone (no external deps beyond `tree_sitter`)
/// and handles its own lint suppression via `#![allow(...)]`. Its doc
/// comments also contain bracketed prose like `[optional]`/`[required]`
/// (emitted by the external generator), which rustdoc would otherwise treat
/// as broken intra-doc links; that lint is suppressed here since the content
/// is generated and regenerable, not authoritative API documentation.
#[allow(missing_docs, non_snake_case, rustdoc::broken_intra_doc_links)]
