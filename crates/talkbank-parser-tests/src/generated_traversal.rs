...
///Typed wrapper for `xphoint_tier_prefix` nodes. Kind is verified at construction.
#[derive(Debug, Clone, Copy)]
...
///Extracted children for `xphoint_dependent_tier` nodes.
#[derive(Debug)]
pub struct XphointDependentTierChildren<'tree> {
    ///`xphoint_tier_prefix`, required
    pub child_0: NodeSlot<'tree, XphointTierPrefixNode<'tree>>,
    ///`tier_sep`, required
    pub child_1: NodeSlot<'tree, TierSepNode<'tree>>,
    ///`text_with_bullets`, required
    pub child_2: NodeSlot<'tree, TextWithBulletsNode<'tree>>,
...
pub trait GrammarTraversal {
    ...
    ///Extract children from a `xphoint_dependent_tier` node.
    ///
    ///Production (children in order):
    ///- `xphoint_tier_prefix` [required]
    ///- `tier_sep` [required]
    ///- `text_with_bullets` [required]
    ///- `newline` [required]
    fn extract_xphoint_dependent_tier<'tree>(
        &mut self,
        node: tree_sitter::Node<'tree>,
    ) -> XphointDependentTierChildren<'tree> {
        let child_count = node.child_count();
        let mut idx: u32 = 0;
        let skip_extras = |node: tree_sitter::Node, idx: &mut u32, count: usize| {
            while (*idx as usize) < count {
        ...
        let mut child_0: NodeSlot<'tree, XphointTierPrefixNode<'tree>> = NodeSlot::Absent;
        let mut child_1: NodeSlot<'tree, TierSepNode<'tree>> = NodeSlot::Absent;
        let mut child_2: NodeSlot<'tree, TextWithBulletsNode<'tree>> = NodeSlot::Absent;
        let mut child_3: NodeSlot<'tree, NewlineNode<'tree>> = NodeSlot::Absent;
        skip_extras(node, &mut idx, child_count);
        if (idx as usize) < child_count {
            if let Some(child) = node.child(idx) {
                child_0 = classify_child(child, "xphoint_tier_prefix", XphointTierPrefixNode);
                idx += 1;
            }
        }
        skip_extras(node, &mut idx, child_count);
