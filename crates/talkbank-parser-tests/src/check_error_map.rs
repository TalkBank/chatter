//! Curated CLAN CHECK error-number <-> chatter `ErrorCode` mapping, and the
//! canonical 1-161 CHECK message table.
//!
//! RECOVERED 2026-06-22 from the dormant `talkbank-clan` crate
//! (`src/commands/check/error_map.rs`), where it had been stranded by the
//! 2026-06-15 folder-move extraction. It is kept CHECK-*validity* data: the
//! single source of truth that maps chatter validation codes to the CLAN CHECK
//! numbers (1-161) defined in `check.cpp`. It REPLACES the parity audit's
//! former message-keyword heuristic, which guessed mappings by shared words and
//! so manufactured false "complete" verdicts (CHECK 126/127 were certified done
//! with no implementation).
//!
//! `check_error_number` is a `match` over `ErrorCode`, so the compiler keeps it
//! co-located with the enum. Unmapped codes return 0 (a genuine gap, not a
//! keyword coincidence).
//!
//! # Divergences
//!
//! Not all CHECK errors have direct equivalents: some are obsolete (e.g. error
//! 96 "Word color is now illegal"), some are subsumed by chatter's parser-level
//! structural validation, and some conditions chatter handles differently.

use talkbank_model::errors::ErrorCode;

/// Map a chatter `ErrorCode` to the closest CLAN CHECK error number (1-161).
///
/// Returns 0 when there is no direct CHECK equivalent.
pub fn check_error_number(code: &ErrorCode) -> u16 {
    match code {
        ...
        // -- Retrace --
        ErrorCode::StructuralOrderError => 119, // Retrace marker must be followed by material
...
/// The canonical CLAN CHECK message for a given error number, taken verbatim
/// from CLAN's `check_mess()` in `check.cpp`.
...
mod tests {
    ...
    #[test]
    fn all_messages_non_empty() {
        for n in 1..=161u16 {
            let msg = check_error_message(n);
            assert!(!msg.is_empty(), "CHECK {n} has empty message");
            assert_ne!(msg, "Unknown error.", "CHECK {n} fell through to fallback");
        }
    }
}
