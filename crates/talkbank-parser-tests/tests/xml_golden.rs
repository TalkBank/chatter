...
const EXPECTED_MISSING_XML_GOLDENS: &[&str] = &[
    ...
    // XML-parser parity: they have no reference XML golden and are not part of
    // the XML parity suite. Adjudicated as intentionally missing here.
    "languages/manchester-anne",
    "languages/manchester-aran",
    "tiers/pho",
    // Phon %xphoint reference fixture. Like the other Phon phonetic tiers it is
...
fn rel_path(base: &Path, path: &Path) -> String {
    // Canonicalize both sides before stripping. rstest's `#[files]` yields
    // canonicalized paths, which on Windows carry the `\\?\` verbatim prefix,
    // whereas `base` is built from CARGO_MANIFEST_DIR without it; component-wise
    // `strip_prefix` then fails because the verbatim prefix is a distinct
    // component (Windows-only; the two forms coincide on macOS/Linux).
    // Canonicalizing both gives them the same form on every platform. Both paths
    // point at real files in the reference corpus, so canonicalize succeeds.
    let base = base
        .canonicalize()
        .expect("reference base directory should exist");
    let path = path
        .canonicalize()
        .expect("reference path should exist under the corpus");
...
///
/// The two trees mirror each other: the XML tree is the reference
/// TalkBank XML for the CHAT reference tree, preserving the
/// relative paths.
