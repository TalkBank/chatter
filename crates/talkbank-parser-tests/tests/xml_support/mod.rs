// `decode_and_unescape_value` is deprecated in current quick-xml in favor of
// `decoded_and_normalized_value(XmlVersion)`. There's no drop-in replacement
// for the un-versioned variant, and the same suppression is already used in
// the converter crates that share this XML reading style.
#![allow(deprecated)]
...
//!
//! The reference TalkBank XML uses a specific layout (indent = two
//! spaces, hard-coded blank lines inside `<participant>`, etc.) that
//! carries no semantic information. Downstream consumers (NLTK ElementTree,
...
/// text is trimmed. Attributes are sorted by name so that the reference
/// TalkBank XML's declared attribute order and the Rust emitter's
/// declared order are treated as equivalent.
...
fn extract_events(xml: &str, label: &str) -> Result<Vec<NormalizedEvent>, XmlStructuralDiff> {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            // for structural equality. The reference TalkBank XML emits
            // <?xml version=...?> and `xsi:schemaLocation` etc. as
            // attributes on <CHAT>, which we do compare.
            Ok(_) => {}
        }
        buf.clear();
    }
