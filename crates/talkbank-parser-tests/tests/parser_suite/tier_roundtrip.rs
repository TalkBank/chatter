...
fn golden_main_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        ...
        println!("[tree-sitter] Testing {} main tiers...", main_tiers.len());

        for (i, tier_line) in main_tiers.iter().enumerate() {
            // Print progress every 100 tiers
            if i > 0 && i % 100 == 0 {
                println!(
                    "[tree-sitter] Progress: {}/{} tiers...",
                    i,
                    main_tiers.len()
                );
            }
            ...
            if !sink.is_empty() {
                ...
                eprintln!("[tree-sitter] Tier #{} FAILED: {}", i, tier_line);
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!(
                "[tree-sitter] \u{2705} All {} context-free main tiers passed! ({} skipped: require file context)",
                main_tiers.len() - skipped_context_tiers,
                skipped_context_tiers
            );
        } else {
            println!("[tree-sitter] \u{274c} {} failed", failed_tiers.len());
        }
    }
...
fn golden_mor_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        println!("[tree-sitter] Testing {} %mor tiers...", mor_tiers.len());
        ...
        for (i, tier_line) in mor_tiers.iter().enumerate() {
            ...
            if !sink.is_empty() {
                let error_msg = format!("Parse errors: {:?}", sink.to_vec());
                eprintln!("[tree-sitter] %mor Tier #{} FAILED: {}", i, tier_line);
                eprintln!("  Error: {}", error_msg);
                failed_tiers.push(("tree-sitter", i, tier_line, error_msg));
                continue;
            }
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!("[tree-sitter] All {} %mor tiers passed!", mor_tiers.len());
        } else {
            println!("[tree-sitter] {} %mor tiers failed", failed_tiers.len());
        }
    }
...
fn golden_gra_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        println!("[tree-sitter] Testing {} %gra tiers...", gra_tiers.len());
        ...
        for (i, tier_line) in gra_tiers.iter().enumerate() {
            ...
            if !sink.is_empty() {
                let error_msg = format!("Parse errors: {:?}", sink.to_vec());
                eprintln!("[tree-sitter] %gra Tier #{} FAILED: {}", i, tier_line);
                eprintln!("  Error: {}", error_msg);
                failed_tiers.push(("tree-sitter", i, tier_line, error_msg));
                continue;
            }
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!("[tree-sitter] All {} %gra tiers passed!", gra_tiers.len());
        } else {
            println!("[tree-sitter] {} %gra tiers failed", failed_tiers.len());
        }
    }
