...
fn golden_pho_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        println!("[tree-sitter] Testing {} %pho tiers...", pho_tiers.len());
        ...
        for (i, tier_line) in pho_tiers.iter().enumerate() {
            ...
            if !sink.is_empty() {
                ...
                eprintln!("[tree-sitter] %pho Tier #{} FAILED: {}", i, tier_line);
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!("[tree-sitter] All {} %pho tiers passed!", pho_tiers.len());
        } else {
            println!("[tree-sitter] {} %pho tiers failed", failed_tiers.len());
        }
    }
...
fn golden_sin_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        println!("[tree-sitter] Testing {} %sin tiers...", sin_tiers.len());
        ...
        for (i, tier_line) in sin_tiers.iter().enumerate() {
            ...
            if !sink.is_empty() {
                let error_msg = format!("Parse errors: {:?}", sink.to_vec());
                eprintln!("[tree-sitter] %sin Tier #{} FAILED: {}", i, tier_line);
                eprintln!("  Error: {}", error_msg);
                failed_tiers.push(("tree-sitter", i, tier_line, error_msg));
                continue;
            }
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!("[tree-sitter] All {} %sin tiers passed!", sin_tiers.len());
        } else {
            println!("[tree-sitter] {} %sin tiers failed", failed_tiers.len());
        }
    }
