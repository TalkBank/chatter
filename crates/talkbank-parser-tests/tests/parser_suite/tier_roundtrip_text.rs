...
fn golden_wor_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        println!("[tree-sitter] Testing {} %wor tiers...", wor_tiers.len());
        ...
        for (i, tier_line) in wor_tiers.iter().enumerate() {
            ...
            if !sink.is_empty() {
                ...
                eprintln!("[tree-sitter] %wor Tier #{} FAILED: {}", i, tier_line);
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!("[tree-sitter] All {} %wor tiers passed!", wor_tiers.len());
        } else {
            println!("[tree-sitter] {} %wor tiers failed", failed_tiers.len());
        }
    }
...
fn golden_com_tier_roundtrip_for_every_parser() -> Result<(), TestError> {
    ...
    for parser in parser_suite()? {
        println!("[tree-sitter] Testing {} %com tiers...", com_tiers.len());
        ...
        for (i, tier_line) in com_tiers.iter().enumerate() {
            ...
            if !sink.is_empty() {
                let error_msg = format!("Parse errors: {:?}", sink.to_vec());
                eprintln!("[tree-sitter] %com Tier #{} FAILED: {}", i, tier_line);
                eprintln!("  Error: {}", error_msg);
                failed_tiers.push(("tree-sitter", i, tier_line, error_msg));
                continue;
            }
  ...
  Roundtrip: {}",
        ...
        if failed_tiers.is_empty() {
            println!("[tree-sitter] All {} %com tiers passed!", com_tiers.len());
        } else {
            println!("[tree-sitter] {} %com tiers failed", failed_tiers.len());
        }
    }
