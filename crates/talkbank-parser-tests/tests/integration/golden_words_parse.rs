...
//! Word-level PARSE tests over the golden word list.
//!
//! Named `parser_equivalence_words` until 2026-08-26, which was false: this
//! crate does not depend on `talkbank-parser-re2c` at all, so nothing here can
//! compare the two backends. The oracle is
...
//! ```bash
//! # Run all word tests
//! cargo test golden_words_parse
//!
//! # Show failing words
//! cargo test golden_words_parse -- --nocapture
//! ```
