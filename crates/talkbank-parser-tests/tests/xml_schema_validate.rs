...
//! ([`xml_golden`](./xml_golden.rs)), parity proves we match the
//! reference TalkBank XML's shape, XSD validation proves the shape is
//! schema-legal. Both gates together guarantee downstream consumers can
//! parse our output.
//!
...
/// `xmllint --schema talkbank.xsd --noout`. Files with no paired
/// `.xml` golden still get validated, if our output is
/// schema-conforming, the parity gap is safely golden-side only.
#[rstest]
