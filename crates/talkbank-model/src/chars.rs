...
//! curly-quote characters CHAT validation flags as illegal, and they
//! were previously inlined as bare `'\u{201C}'`-style char literals at
//! every use site across `talkbank-model`, `talkbank-parser`,
//! the re2c parser, and downstream consumers. That scatter made the
//! codepoints' CHAT semantics
//! invisible at the call site (a reader
...
///
/// CHAT role: opening bookend of a quotation group (`QuotationGroup`) in the
/// main tier, paired with [`RIGHT_DOUBLE_QUOTE`]. Inside ASR-emitted word
...
///
/// CHAT role: closing bookend of a quotation group (`QuotationGroup`) in the
/// main tier, paired with [`LEFT_DOUBLE_QUOTE`]. Same boundary-noise
