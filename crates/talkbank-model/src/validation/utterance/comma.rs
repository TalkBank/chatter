...
use crate::model::{Separator, Utterance, Word};
...
fn is_comma_licensing(item: &ContentItem<'_>) -> bool {
    match item {
        ContentItem::Word(word) => is_real_word(word),
        ContentItem::ReplacedWord(replaced) => is_real_word(&replaced.word),
        // Pauses license commas (matching CLAN CHECK behavior).
        ContentItem::Pause(_) => true,
        ...
        | ContentItem::OtherSpokenEvent(_) => false,
    }
}
...
pub(crate) fn check_comma_after_non_spoken(utterance: &Utterance, errors: &impl ErrorSink) {
    ...
    let mut context = CommaContext::AwaitingContent;

    walk_content(&utterance.main.content.content, None, &mut |item| {
        if is_comma_licensing(&item) {
            context = CommaContext::Licensed;
            return;
        }

        if let ContentItem::Separator(Separator::Comma { span }) = item
            && matches!(context, CommaContext::AwaitingContent)
        {
            errors.report(
                ParseError::new(
