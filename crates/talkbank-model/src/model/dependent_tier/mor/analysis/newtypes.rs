...
impl PosCategory {
    /// Separator between the coarse category head and each
    /// subcategory refinement in a CHAT POS tag (`n:prop`,
    /// `pro:poss:det`).
    const SUBCATEGORY_SEPARATOR: char = ':';

    /// The coarse category head: the text before the first `:`.
    ///
    /// CHAT POS tags may carry colon-separated subcategory
    ...
    pub fn category(&self) -> &str {
        match self.0.split_once(Self::SUBCATEGORY_SEPARATOR) {
            Some((head, _refinements)) => head,
            None => &self.0,
        }
    }

    /// The subcategory refinements: each colon-separated segment
    /// after [`Self::category`], in order. Empty for an unrefined
    /// tag (`n`); yields `["prop"]` for `n:prop` and
    /// `["poss", "det"]` for `pro:poss:det`.
    ///
    /// Segments are returned exactly as stored; a degenerate tag
    /// with an empty segment (`n:`) yields that empty segment, and
    /// `%mor` validation is responsible for flagging it.
    pub fn subcategories(&self) -> impl Iterator<Item = &str> {
        self.0.split(Self::SUBCATEGORY_SEPARATOR).skip(1)
    }
}
...
mod tests {
    ...
    /// An unrefined tag is its own category head with no subcategories.
    ///
    /// This is the common `%mor` case (`n|dog`, `v|run`).
    #[test]
    fn test_pos_category_unrefined() {
        let pos = PosCategory::new("n");
        assert_eq!(pos.category(), "n");
        assert_eq!(pos.subcategories().count(), 0);
    }
    ...
    fn test_pos_category_single_subcategory() {
        ...
        assert_eq!(pos.category(), "n");
        assert_eq!(pos.subcategories().collect::<Vec<_>>(), vec!["let"]);
    }

    /// Nested refinements yield every segment after the head, in order.
    ///
    /// Real data carries chains like `pro:poss:det` (CHILDES English MOR).
    #[test]
    fn test_pos_category_nested_subcategories() {
        let pos = PosCategory::new("pro:poss:det");
        assert_eq!(pos.category(), "pro");
        assert_eq!(pos.subcategories().collect::<Vec<_>>(), vec!["poss", "det"]);
    }
