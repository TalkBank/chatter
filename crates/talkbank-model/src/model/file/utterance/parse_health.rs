...
impl ParseHealth {
    ...
    /// Returns whether `%xphoint` to `%pho` checks are safe to run.
    ///
    /// Both tiers must be parse-clean for per-phone interval validation.
    pub fn can_align_xphoint_to_pho(&self) -> bool {
        self.is_tier_clean(ParseHealthTier::Xphoint) && self.is_tier_clean(ParseHealthTier::Pho)
    }
...
impl ParseHealthState {
    ...
    /// Returns `true` when `%xphoint` to `%pho` checks are safe to run.
    pub fn can_align_xphoint_to_pho(self) -> bool {
        match self {
            Self::Unknown => false,
            Self::Clean => true,
