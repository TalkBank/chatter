...
// Same Apple-Event side-effect hazard as test_concurrent_send_attempts_no_crash:
// on a developer machine with CLAN.app installed and running, the Event reaches
// CLAN.app which pops a dialog about the missing /tmp file. Mark #[ignore] for
// the same reason; CI without CLAN.app gets the no-panic guarantee via the
// underlying FFI's "app not found" return path which doesn't need this test.
#[test]
#[ignore = "spawns CLAN.app dialog on machines where CLAN is installed; opt-in via --ignored"]
...
// Same hazard, empty path still goes through the FFI to CLAN.app.
#[test]
#[ignore = "spawns CLAN.app dialog on machines where CLAN is installed; opt-in via --ignored"]
...
// Side effect on developer machines with CLAN.app installed: the Apple
// Event reaches a running CLAN.app and pops a "can't open file" dialog
// because the /tmp paths don't exist. The test passes (no crash, no
// panic, both threads join), but it spams the desktop. CI runs without
// CLAN.app installed so the events fail at the runtime-not-running
// check and the test is a no-op; that's fine for the no-crash property.
// Marked #[ignore] so it doesn't fire under `cargo test`; run with
// `cargo test -- --ignored` when explicitly exercising the FFI on a
// machine with CLAN intentionally running for the test.
#[test]
#[ignore = "spawns CLAN.app dialog on machines where CLAN is installed; opt-in via --ignored"]
