...
///
/// `Clean` and `Normal` are kept as derive inputs to exercise the
/// "no suffix → default tag" branch of the convention resolver; the
/// tests construct only `ParseError` / `DeferredWarning` to assert the
/// suffix-driven tagging.
#[derive(Debug, Clone, DeriveValidationTagged)]
#[allow(dead_code)]
