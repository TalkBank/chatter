...
`dist-workspace.toml` sets `create-release = false`, `github-release =
"announce"`, `publish-jobs = ["./release-desktop"]` and `post-announce-jobs =
["./release-app-banner"]`. After the CLI artifacts build, `release.yml` calls
