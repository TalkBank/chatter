...
- **The desktop version lives in two places.** `apps/chatter-desktop/src-tauri/Cargo.toml`
  inherits the workspace version, but `apps/chatter-desktop/src-tauri/tauri.conf.json`
  carries its own literal `"version"`. Bump BOTH on a release, or the `.dmg` and the Tauri
  updater report a stale version while the CLI is current.
...
3. Regenerate typed CST traversal, requires a local checkout of the
   ...
   `crates/talkbank-parser-tests/src/generated_traversal.rs`. See
   that crate's `lib.rs` doc-comment for the exact command shape.
