...
pub fn generate_error_test(
    ...
    // (gen_validation_corpus + the data-driven validation_error_corpus.rs
...
/// Clean this generator's outputs from `output_dir`, then write them.
///
/// Cleans by NAME, deliberately, rather than by owning the directory.
/// `tests/integration/generated/` has TWO producers: this generator, and
/// `bootstrap_reference_corpus` which writes `reference_corpus.rs`. Claiming
/// the directory deletes the other producer's output, which broke the build
/// immediately when it was tried on 2026-07-29.
///
/// Cost of the name list, accepted: it cannot remove stale SUBdirectories, so a
/// deleted spec can leave an orphan behind. Give each producer its own owned
/// directory if that ever matters.
pub fn write_generated_tests(
    ...
    if output_dir.exists() {
        let sweepable: Vec<&str> = GeneratedTestFile::ALL
            .iter()
            .map(|file| file.file_name())
            .chain(RETIRED_OUTPUT_NAMES.iter().copied())
            .collect();
        for entry in std::fs::read_dir(output_dir)? {
            ...
            let is_ours = path
                ...
                .is_some_and(|name| sweepable.contains(&name));
            if is_ours {
    ...
    let mut written = Vec::with_capacity(GeneratedTestFile::ALL.len());
    for file in GeneratedTestFile::ALL {
