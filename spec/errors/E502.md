...
**Status:** Current
**Last updated:** 2026-09-06 20:10 EDT
...
Validation reports E502 once, at the end of the file, when no `@End` was
parsed. A file ending in `@End` is not reported.

For the example above, recovery retains the complete final main tier in the
partial model, with or without a final newline. The missing document
terminator must not erase otherwise recognizable speech or introduce
unrelated syntax diagnostics.
...
- Until 2026-09-03 this code had two spec files; the second was a bug
  report about a false E502 caused by tree-sitter's error recovery
  swallowing `@End` after a long invalid `%wor` line. It carried no
  frontmatter examples; its text is kept below as history and should be
  re-verified against the current grammar before being believed.

### History: the %wor cascade false positive (recorded 2026-04)
