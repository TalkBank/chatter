+++
code = 'E725'
name = 'Modsyl tier word count does not match mod tier'

[[example]]
level = 'tier'
claim = 'violates'
chat = '''
@UTF8
@Begin
@Languages:	nld
@Participants:	CHI Child, MOT Mother
@ID:	nld|corpus|CHI|2;00.||||Child|||
@ID:	nld|corpus|MOT|||||Mother|||
*MOT:	muts ja .
%mod:	ˈmœts ˈja
%xmodsyl:	ˈm:Oœ:Nt:Cs:R
@Comment:	ERROR: %mod has 2 words but %xmodsyl has only 1
@End
'''
[[example]]
title = 'Two complete source and syllabification words'
level = 'tier'
claim = 'legal'
chat = '''
@UTF8
@Begin
@Languages:	nld
@Participants:	CHI Child, MOT Mother
@ID:	nld|corpus|CHI|2;00.||||Child|||
@ID:	nld|corpus|MOT|||||Mother|||
*MOT:	muts ja .
%mod:	ˈmœts ˈja
%xmodsyl:	ˈm:Oœ:Nt:Cs:R ˈj:Oa:N
@End
'''

[[example]]
title = 'Delete the final source word, retaining both syllabification words'
level = 'tier'
claim = 'violates'
notes = 'Derived from the preceding complete control by deleting only the space and final %mod word. The unmatched syllabification word has no source text to reconstruct; E725 diagnoses cardinality without fabricating an E737 comparison.'
chat = '''
@UTF8
@Begin
@Languages:	nld
@Participants:	CHI Child, MOT Mother
@ID:	nld|corpus|CHI|2;00.||||Child|||
@ID:	nld|corpus|MOT|||||Mother|||
*MOT:	muts ja .
%mod:	ˈmœts
%xmodsyl:	ˈm:Oœ:Nt:Cs:R ˈj:Oa:N
@End
'''
+++

## Description

The `%xmodsyl` tier word count does not match the `%mod` tier word count. Each word-level entry in `%xmodsyl` must correspond one-to-one with a word-level entry in `%mod`.

## Expected Behavior

- **Parser**: Should succeed, syntax is valid
- **Validator**: Should report E725, `%xmodsyl` word count (1) does not match `%mod` word count (2)

## CHAT Rule

The `%xmodsyl` tier provides syllable-level morphological annotation and must have the same number of word-level entries as the corresponding `%mod` tier.

The paired source-deletion examples distinguish a count mismatch from a
reconstruction mismatch: an extra syllabification word still violates E725,
but without a source word at that position there is no text for E737 to compare.
No empty or fabricated source word may stand in for that absence.

## Notes

Validated in `alignment.rs` via `build_tier_to_tier_alignment()` which compares word counts between `%xmodsyl` and `%mod` tiers within the same utterance.
