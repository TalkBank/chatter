...
pub fn to_frontend_event(event: ValidationEvent, root: &Path) -> Option<FrontendEvent> {
    match event {
        ...
        ValidationEvent::Errors(e) => {
            ...
            // Route through the SAME shared orchestration the CLI uses, so the
            // GUI cannot diverge from the CLI on how an error is enhanced or
            // which source line the caret lands on. `Ansi` mode yields both the
            // plain text (for "Copy") and the colored form (converted to HTML).
            let diagnostics = render_diagnostics(&e.errors, &file_path, &source, RenderMode::Ansi)
                .into_iter()
                .map(|d| {
                    let rendered_html = match &d.ansi {
                        Some(ansi) => ansi_to_html(ansi),
                        // RenderMode::Ansi always populates `ansi`; escape the
                        // plain text rather than panic or drop the diagnostic if
                        // that invariant ever changes.
                        None => ansi_to_html(&d.text),
                    };
                    FrontendDiagnostic {
                        // The ENHANCED error (line/column populated): strictly
                        // more useful to the frontend and to Open-in-CLAN than
                        // the raw pre-enhancement error sent previously.
                        error: d.error,
                        rendered_html,
