...
export const tauriTransport: DesktopTransport = {
  ...
  async checkForUpdate(): Promise<AvailableUpdate | null> {
    // Updating only applies to the installed native build, not the web/dev
    // shell, so there is nothing to check there.
    if (!isNativeDesktop()) {
      return null;
    }
  ...
  async askInstallUpdate(version, currentVersion, notes) {
    const detail = notes ? `\n\n${notes}` : "";
    return ask(
